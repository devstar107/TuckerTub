...
import type { AccountBillingInput } from '~/validation/account-billing.schema'
import { accountBillingSchema } from '~/validation/account-billing.schema'
...
export const AccountBilling = () => {
  ...
  console.log('AccountBilling-parsedCustomercustomerInfo', customer)
  ...
  } = useForm<AccountBillingInput>({
    resolver: zodResolver(accountBillingSchema),
    mode: 'all'
  })
  ...
  useEffect(() => {
    if (customer?.billing?.first_name) {
      ...
      if (values.suburb?.length === 0) {
        setValue('suburb', customer?.billing?.city)
      }

      if (values.state?.length === 0) {
        setValue('state', customer?.billing?.state)
      }
  ...
  async function onSubmitForm(values: AccountBillingInput) {
    try {
      ...
      console.log('AccountBilling-Data', data)

      if (response.ok) {
        setSuccessMessage('Your billing details have been updated')
        localStorage.setItem(WOOCOMMERCE_TUCKERTUB_LOCALSTORAGE_CUSTOMER_INFO, JSON.stringify(data))
      } else {
        throw new Error(data)
      }
    } catch (error) {
      console.log('AccountBilling-Error', error)
      setSuccessMessage('')
      setAuthErrorMessage('Something went wrong')
    }
  }
